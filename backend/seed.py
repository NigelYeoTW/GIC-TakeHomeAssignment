...
def seed():
    ...
    try:
        ...
        assignments = [
            CafeEmployee(cafe_id=cafes[0].id, employee_id="UIAB12345", start_date=datetime.now(timezone.utc).date() - timedelta(days=120)),
            CafeEmployee(cafe_id=cafes[0].id, employee_id="UIBC23456", start_date=datetime.now(timezone.utc).date() - timedelta(days=60)),
            CafeEmployee(cafe_id=cafes[1].id, employee_id="UICD34567", start_date=datetime.now(timezone.utc).date() - timedelta(days=200)),
            CafeEmployee(cafe_id=cafes[1].id, employee_id="UIDE45678", start_date=datetime.now(timezone.utc).date() - timedelta(days=30)),
            # Eva is unassigned
        ]
